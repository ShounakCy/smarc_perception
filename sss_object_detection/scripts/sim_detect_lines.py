...
class sim_sss_detector:
    ...
    def _update_marked_positions(self, msg):
        ...
        if len(self.marked_positions) > 0:
            return
        print >>sys.stderr, 'Marked msg.markers of = "%s"'  % msg.markers
        for marker in msg.markers:
    ...
    '''
    def _get_distance_to_marker(self, marker, msg):
        """Return distance from the marker to current_pose"""
        print >>sys.stderr, 'marker = "%s"'  % marker
        #marker_copy = marker
        #P = self.detect_projected_points(msg)
        #marker_copy.pose.position.x = P[0]
        #marker_copy.pose.positon.y = P[1]
        #print >>sys.stderr, 'P_marker = "%s"'  % marker_copy
